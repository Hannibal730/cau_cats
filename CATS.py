...
class Model_backbone(nn.Module):
    ...
    def __init__(self, c_in:int, seq_len:int, pred_len:int, featured_patch_dim:int=24, stride:int=24, n_layers:int=3, d_model=128, n_heads=16, d_ff:int=256,
        ...
        # (pred_len + featured_patch_dim - 1) // featured_patch_dim은 math.ceil(pred_len / featured_patch_dim)과 동일한 결과를 정수 연산으로 수행합니다.
        ...
        self.proj = Projection(d_model, self.featured_patch_dim)
        # 백본의 출력(`d_model` 차원)을 최종 예측 패치(`featured_patch_dim` 차원)로 변환하는 프로젝션 레이어를 생성합니다.
...
class Learnable_Query_Embedding(nn.Module): 
    # 클래스의 생성자입니다.
    def __init__(self, c_in, seq_patch_num, featured_patch_dim, pred_patch_num, n_layers=3, d_model=128, n_heads=16, QAM_start=0.1, QAM_end=0.5, d_ff=256, 
                 attn_dropout=0., dropout=0., store_attn=False, res_attention=True, channel_independence=False, positional_encoding=True, **kwargs):
             
        super().__init__()
        # `nn.Module`의 생성자를 호출합니다.
        
        # --- 입력 인코딩 ---
        self.W_P = nn.Linear(featured_patch_dim, d_model)      
        # 입력 패치(`featured_patch_dim` 차원)를 모델의 은닉 상태 차원(`d_model`)으로 변환하는 선형 레이어(가중치 `W_P`)를 정의합니다.
        ...
        if self.channel_independence:
            # 만약 변수들이 독립적이라면,
            self.learnable_queries = nn.Parameter(0.5*torch.randn(pred_patch_num, featured_patch_dim))
            ...
            # 크기는 [예측 패치 수, featured_patch_dim]입니다.
        else:
            # 변수들이 서로 종속적이라면,
            self.learnable_queries = nn.Parameter(0.5*torch.randn(c_in, pred_patch_num, featured_patch_dim))
            # 각 변수별로 별도의 쿼리 셋을 학습 가능한 파라미터(nn.Parameter)로 생성합니다.
            # 크기는 [변수 수, 예측 패치 수, featured_patch_dim]입니다.
    ...
    def forward(self, x) -> Tensor:
        # 입력 x의 형태: [배치 크기, 변수 수, 시퀀스 패치 수, featured_patch_dim]
        ...
        # --- 1. 디코더에 입력할 입력 시퀀스 준비. seq_patch: [배치 크기*변수 수, 시퀀스 패치 수, d_model] 구하기---
        x = self.W_P(x)
        if self.use_positional_encoding:
            x = x + self.PE
        # 입력 패치에 Positional Encoding을 추가해주는 과정이다.
        # 입력 패치 `x`를 선형 레이어 `W_P(featured_patch_dim을 d_model로 변환)`로 임베딩하면 결과 x의 형태: [배치 크기, 변수 수, 시퀀스 패치 수, d_model] 
        ...
        # 결과 learnable_queries의 형태: (독립적일 때) [예측 패치 수, d_model]. 왜냐하면 self.learnable_queries = nn.Parameter(0.5*torch.randn(pred_patch_num, featured_patch_dim))
        # 결과 learnable_queries의 형태: (종속적일 때) [변수 수, 예측 패치 수, d_model]. 왜냐하면 self.learnable_queries = nn.Parameter(0.5*torch.randn(c_in, pred_patch_num, featured_patch_dim))
...
class Projection(nn.Module):
    # 프로젝션 클래스의 생성자입니다.
    def __init__(self, d_model, featured_patch_dim):
        super().__init__()
        # `nn.Module`의 생성자를 호출합니다.
        self.linear = nn.Linear(d_model, featured_patch_dim)
        # 트랜스포머의 은닉 상태 차원, 즉 임베딩 차원(`d_model`)을 `featured_patch_dim`으로 변환하는 선형 레이어를 정의합니다.
        self.flatten = nn.Flatten(start_dim=-2)
        # 예측 패치들을 하나의 연속된 시계열로 펼치기 위한 Flatten 레이어를 정의합니다.
        # `start_dim=-2`는 마지막에서 두 번째 차원부터 평탄화를 시작하도록 지정합니다.
        # [배치 크기, 변수 수, pred_patch_num, featured_patch_dim] -> [배치 크기, 변수 수, pred_patch_num * featured_patch_dim]
    ...
    def forward(self, x):
        ...
        # linear 층 거친 후 결과: [배치 크기, 변수 수, 예측 패치 수, featured_patch_dim]
        
        # flatten을 적용하여 [B, C, pred_patch_num * dim] 형태로 반환
        x = self.flatten(x)
        return x
